...
import axios from 'axios'; import { exec, execSync } from 'child_process'; import fs from 'fs'; import path from 'path'; import os from 'os'; import fileType from 'file-type'; import googleTTS from 'google-tts-api'; import { downloadMediaMessage } from '@whiskeysockets/baileys';

// Helper: Get temp file path function getTempFilePath(ext = '') { return path.join(os.tmpdir(), liambot_${Date.now()}${ext}); }

// 🧨 .sticker command export async function sticker(sock, msg) { const chatId = msg.key.remoteJid; try { const quoted = msg.message?.extendedTextMessage?.contextInfo?.quotedMessage; if (!quoted) { return await sock.sendMessage(chatId, { text: "🖼️ Sticker Maker\n_Reply to an image or short video with .sticker_ to create a cool sticker!", }); }
...
} catch (err) { console.error(err); await sock.sendMessage(chatId, { text: "❌ Error creating sticker. Try again with a clear image or short video (under 10s).", }); } }

// 🎥 .youtube command export async function youtube(sock, msg, args) { const chatId = msg.key.remoteJid; const query = args.join(' ').trim();

if (!query) { return await sock.sendMessage(chatId, { text: "🎥 YouTube Downloader\nUsage: .youtube [search or link]", }); }

const outPath = getTempFilePath('.mp4'); const searchParam = query.includes('http') ? query : ytsearch1:${query}; const cmd = yt-dlp -f "best[ext=mp4]" --write-thumbnail --write-info-json -o "${outPath}" "${searchParam}";
...
} catch (e) { console.error(e); await sock.sendMessage(chatId, { text: ❌ Error: ${e.message} }); } }

// 🎵 .play command export async function play(sock, msg, args) { const chatId = msg.key.remoteJid; const query = args.join(' ').trim();

if (!query) { return await sock.sendMessage(chatId, { text: "🎶 Play Music\nUsage: .play [song or link]\n_Example:_ .play Blinding Lights", }); }
...
// 🐸 .meme command export async function meme(sock, msg) { const chatId = msg.key.remoteJid; try { const res = await axios.get('https://meme-api.com/gimme'); const meme = res.data;
...
} catch (err) { console.error(err); await sock.sendMessage(chatId, { text: "😓 Meme gods are asleep. Try again in a bit!", }); } }

// 🎞️ .memegif command export async function memegif(sock, msg) { const chatId = msg.key.remoteJid; try { let tries = 0; let meme, buffer, type;
...
} catch (err) { console.error(err); await sock.sendMessage(chatId, { text: "😭 No GIF memes found. Try again later!", }); } }

// 🔊 .tts command export async function tts(sock, msg, args) { const chatId = msg.key.remoteJid; const text = args.join(' ').trim();

if (!text) { return await sock.sendMessage(chatId, { text: "🔊 Text-to-Speech\nUsage: .tts Hello world\nSupported language: English only", }); }
...
} catch (err) { console.error(err); await sock.sendMessage(chatId, { text: "❌ Failed to generate speech. Try again later.", }); } }
