...
// 📽️ .youtube command export async function youtube(sock, msg, args) { const chatId = msg.key.remoteJid; const url = args.join(' ').trim();

if (!url || !url.includes('youtu')) { return sock.sendMessage(chatId, { text: "📽️ YouTube Downloader\n_Usage:_ .youtube https://youtu.be/xyz (under 30 mins)", }); }

const outPath = getTempFilePath('.mp4'); const cmd = yt-dlp -f \"best[ext=mp4][duration<=1800]\" --max-filesize 200M -o \"${outPath}\" \"${url}\";
...
const outPath = getTempFilePath('.mp3'); const ytQuery = query.includes('youtube.com') || query.includes('youtu.be') ? query : ytsearch1:${query};
...
// 🐸 .meme command export async function meme(sock, msg) { const chatId = msg.key.remoteJid; try { const { data: meme } = await axios.get('https://meme-api.com/gimme');
