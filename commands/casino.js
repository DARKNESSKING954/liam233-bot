...
export default {
  ...
  slot(sock, msg, args) {
    ...
    if (!bet || bet <= 0) 
      return sock.sendMessage(msg.key.remoteJid, { text: "Usage: .slot [bet]" });
    if (memory.getWallet(userId) < bet) 
      return sock.sendMessage(msg.key.remoteJid, { text: "💸 Not enough coins." });
  ...
  coinflip(sock, msg, args) {
    ...
    if (!["heads", "tails"].includes(choice)) 
      return sock.sendMessage(msg.key.remoteJid, { text: "Pick heads or tails." });
    if (!bet || bet <= 0) 
      return sock.sendMessage(msg.key.remoteJid, { text: "Invalid bet." });
    if (memory.getWallet(userId) < bet) 
      return sock.sendMessage(msg.key.remoteJid, { text: "💸 Not enough coins." });
  ...
  dice(sock, msg, args) {
    const userId = msg.key.participant || msg.key.remoteJid;
    if (args.length < 2) 
      return sock.sendMessage(msg.key.remoteJid, { text: "Usage: .dice [1-6] [bet]" });
    const guess = parseInt(args[0]);
    const bet = parseInt(args[1]);
    if (guess < 1 || guess > 6 || !bet || bet <= 0) 
      return sock.sendMessage(msg.key.remoteJid, { text: "Invalid guess or bet." });
    if (memory.getWallet(userId) < bet) 
      return sock.sendMessage(msg.key.remoteJid, { text: "💸 Not enough coins." });
  ...
  roulette(sock, msg, args) {
    const userId = msg.key.participant || msg.key.remoteJid;
    if (args.length < 2) 
      return sock.sendMessage(msg.key.remoteJid, { text: "Usage: .roulette [red/black/green/number] [bet]" });
  ...
  blackjack(sock, msg, args) {
    ...
    const bet = parseInt(args[0]);
    if (!bet || bet <= 0) 
      return sock.sendMessage(msg.key.remoteJid, { text: "Usage: .blackjack [bet]" });
    if (memory.getWallet(userId) < bet) 
      return sock.sendMessage(msg.key.remoteJid, { text: "💸 Not enough coins." });
  ...
  lottery(sock, msg, args) {
    ...
    const bet = parseInt(args[0]);
    if (!bet || bet <= 0) 
      return sock.sendMessage(msg.key.remoteJid, { text: "Usage: .lottery [bet]" });
    if (memory.getWallet(userId) < bet) 
      return sock.sendMessage(msg.key.remoteJid, { text: "💸 Not enough coins." });
  ...
  scratch(sock, msg) {
    const userId = msg.key.participant || msg.key.remoteJid;
    const cost = 200;
    if (memory.getWallet(userId) < cost) 
      return sock.sendMessage(msg.key.remoteJid, { text: "💸 Not enough coins." });
